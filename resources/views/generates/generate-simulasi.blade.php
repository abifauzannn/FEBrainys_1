...
@section('title', 'Templat Materi Simulasi - Brainys')
...
@section('content')
    ...
    @php
        $skema = [['value' => 'kelompok', 'label' => 'Kelompok'], ['value' => 'individu', 'label' => 'Individu']];
    @endphp
    ...
    <x-page-title url="{{ route('dashboard') }}" title="Templat Materi Simulasi"
        description="Gunakan template materi berbasis simulasi" />
    ...
    <div class="container flex flex-col px-3 mx-auto sm:px-10 lg:flex-row">
        <div class="w-full lg:w-[500px] flex-col justify-start items-start sm:gap-6 inline-flex h-auto">
            <form action="{{ route('generateSimulasi') }}" method="post" class="w-full" id="simulasiForm">
                @csrf

                <x-generate-field type="text" id="name" name="name" label="Nama Simulasi"
                    placeholder="masukan nama simulasi" tooltipId="nameTooltip"
                ...
                <x-generate-field type="text" id="material" name="material" label="Materi Pelajaran"
                    placeholder="masukan materi pelajaran" tooltipId="materialTooltip"
                ...
                <x-textarea-field id="notes" name="notes" label="Deskripsi Materi" tooltipId="descriptionTooltip"
                    placeholder="masukan deskripsi materi simulasi"
                    tooltipText="Contoh : Untuk memperkenalkan ke siswa tentang bagaimana solusi mengatasi pemanasan global, penyebab pemanasan global dan langkah-langkah mengatasinya" />
    ...
    <script>
        window.addEventListener('load', function() {
            $.ajax({
                url: "{{ route('get.credit.charges.simulasi') }}",
                type: "GET",
        ...
        document.getElementById('simulasiForm').addEventListener('submit', function(event) {
