...
@if ($status == 'baru' || $status == 'old')
    <button @if (!$isDisabled) onclick="{{ $onclick }}" @endif
        ...
        <div class="flex items-center justify-between w-full">
            ...
            @elseif ($status == 'cs')
                <span class="bg-[#88F2C8] text-black text-xs font-bold px-2 py-1 rounded-full -mt-5">SEGERA</span>
            @endif
        </div>
...
@elseif ($status == 'cs')
    <button @if (!$isDisabled) onclick="{{ $onclick }}" @endif
        class=" opacity-50 p-4 rounded-lg border border-gray-300 transition duration-300 ease-in-out flex flex-col items-start text-left shadow-md
        {{ $isDisabled ? '' : 'hover:bg-slate-50' }}"
        {{ $isDisabled ? 'disabled' : '' }}>
        <div class="flex items-center justify-between w-full">
            <img src="{{ $icon }}" alt="" class="w-10 h-10">
